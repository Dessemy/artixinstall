...
draw_ansi_segment(Monitor *m, const char *text, int *x, int height, int lrpad)
{
    ...
    while (text[rd] && wr < (int)sizeof(buffer) - 1) {
        if (text[rd] == '\033' && text[rd + 1] == '[') {
            ...
            const char *seq_end = strchr(text + rd, 'm');
            if (seq_end) {
...
/* Cycles through every entry in layouts[] in order (tile -> float -> monocle
 * -> tile -> ...), unlike setlayout(NULL) which only toggles between
 * whichever two layouts were last used. */
void
